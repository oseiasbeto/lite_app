<template>
    <div :class="{ 'opacity-0 pointer-events-none': !show, 'opacity-100 pointer-events-auto': show }"
        class="absolute w-full z-[99] transition-opacity duration-300 ease-in-out text-white top-0 bg-black/[0.8]">
        <div class="flex items-center justify-between px-1">
            <button @click="$emit('close')"
                class="w-[38px] px-[2.5px] h-[38px] flex justify-center items-center hover:bg-x-dark-surfaceHover rounded-full">
                <svg width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path d="m5.5 5.5 13 13m-13 0 13-13" class="icon_svg-stroke" stroke="#fff" stroke-width="2"
                        fill="none" fill-rule="evenodd" stroke-linecap="round"></path>
                </svg>
            </button>
            <button @click="$emit('onDonwload')"
                class="w-[38px] px-[2.5px] h-[38px] flex justify-center items-center hover:bg-x-dark-surfaceHover rounded-full">
        ...
        <div v-if="author?._id" class="flex px-4 gap-2.5 py-2 items-center flex-row">
            <div class="shrink-0">
                <Avatar :is-darkoo="true" size="md"
                    :url="author?.profile_image?.thumbnails?.sm || author?.profile_image?.url" />
            </div>
            <div class="flex-1">
                    ...
                    <div class="flex items-center gap-1">
                        <div class="flex items-center">
                            ...
                            <svg v-if="author?.is_verified" class="ml-[5px] mb-0.5" fill="none" width="14"
                                ...
                                <path class="text-[rgba(0,0,0,0.8)]" fill="currentColor" fill-rule="evenodd"
                                    clip-rule="evenodd"
