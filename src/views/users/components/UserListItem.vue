<template>
    <div class="bg-transparent active:bg-x-light-surfaceActive dark:active:bg-x-dark-surfaceActive flex items-center py-2 px-4 cursor-pointer h-16 box-border" @click="$emit('click')">
        <Avatar :url="user.profile_image?.thumbnails?.md || user?.profile_image?.url"
            :alt="user.display_name || user.name || ('@' + user.username)" size="lg" class="mr-3 shrink-0 w-[42px] h-[42px]" 
        />
        ...
        <div class="flex-1 mr-2 min-w-0">
            <p class="flex items-center truncate">
                ...
                <svg v-if="user?.is_verified" :class="{ 'w-[12px] h-[12px]': isParentPost }" viewBox="0 0 22 22"
                    aria-label="Verified account" role="img" class="w-[16px] h-[16px] text-x-light-blue ml-[3px]"
        ...
        <button v-if="showBtnFollow && user?._id !== currentUser?._id" @click.stop="handleFollowUser(user?._id)" @mouseenter="isHovering = true"
            ...
            <SpinnerSmall v-if="isFollowingUser" class="mx-auto" />
            <template v-else>
                {{ hasFollowingUser ? (isHovering ? 'A seguir' : 'Seguindo') : 'Seguir' }}
            </template>
        </button>
    </div>
</template>
...
const emit = defineEmits(['click', 'user-followed']);

const store = useStore();
const currentUser = computed(() => store.getters.currentUser);

const hasFollowingUser = computed(() => {
    return !!currentUser.value.following?.includes(props?.user?._id);
});

const isFollowingUser = ref(false); // loading da requisição, não o estado em si
const isHovering = ref(false);

const handleFollowUser = async (userId) => {
    isFollowingUser.value = true;
    await store.dispatch('followUser', { userId, currentUserId: props?.user?._id }).finally(() => {
